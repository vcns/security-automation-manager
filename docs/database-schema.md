...
| v46 | no new table -- bumped purely to re-run `Activator::migrate_remove_direct_stripe_options()` on every already-upgraded site. `includes/extensions/fully-automatic-mode.php`'s direct-Stripe checkout path (secret/price/webhook option storage, the AJAX checkout handler) was removed (`docs/threat-model.md` "Stripe secret storage" finding; `docs/sam-portal-requirements-spec.md` §21.2) -- this migration actively deletes any of those 8 option values a previous private/commercial build may have stored, since they otherwise had no way to be cleared once the settings form that wrote them no longer exists. Only ever mattered on a private/commercial build; neither public release channel has shipped a working checkout path. |
