...
- **Blast radius is scoped to the directive it's added to.** CSP directives don't cross-contaminate: `'unsafe-hashes'` present in `style-src-attr`'s value has no effect on `script-src-attr`, `script-src`, or any script-execution path. `script-src-attr` is `'none'` plus a single hardcoded exception (see below) everywhere in this codebase; inline event handlers with any actual content stay fully blocked regardless of this entry's state.
- **Near-no-op without a paired hash.** The entry only adds the keyword; beyond one hardcoded exception (below), `style-src-attr` still needs an actual approved hash (from `Hash_Manager`'s attribute capture, `docs/database-schema.md`'s `csp_hash_inventory` `style-src-attr` rows) before anything else can match. Enabling the flag with zero approved hashes permits only that one exception, nothing else.
- **The one hardcoded exception: an empty attribute value.** `Policy_Builder::EMPTY_CONTENT_HASH_TOKEN` (the sha256 hash of the empty string) is always present in both `script-src-attr` and `style-src-attr`, unconditionally -- not gated by this bypass toggle or by any capture/approval workflow. An empty attribute value can never execute or style anything: there is no content for it to be an attack vector for, in any context, regardless of what else the page does. Added after a real false-positive: a vendored carousel library's routine `jQuery.attr('style', '')` (clearing inline styles as part of its own destroy/refresh cycle) was reported as a CSP violation purely because that specific empty value had never happened to go through the hash-learning workflow yet -- noise with no security value on either side. Like any other attribute-context hash, it still only takes effect once `'unsafe-hashes'` is present, so this doesn't weaken the "nothing auto-enables" invariant above; it just means this one specific, provably harmless value never needs individual capture and approval.
