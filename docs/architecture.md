...
- **Information Masking** (`Information_Masking_Builder`) -- per-surface on/off. `header_remove()` for `X-Powered-By`, `Server`, and `X-Pingback`; nothing to configure beyond enabling it.
- **Cache-Control** (`Cache_Control_Builder`) -- per-surface preset (`no-store` / `private-no-cache` / `public-short` / `public-long`). `Cache_Control_Conflict_Detector` gates emission: if another caching mechanism (a caching plugin or CDN) is already active, this builder defers instead of sending a competing value.
