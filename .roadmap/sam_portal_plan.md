...
- **Create the `vcns/sam-portal` repository.** Ownership, CI/CD, environments (staging/production, matching `sam-licensing-service`'s existing pattern), architecture decision records. *Stack decided -- §5.1: Cloudflare Workers, matching `sam-licensing-service`.* The sequencing gate (Phase 4F) has now passed -- this bullet is unblocked, not "must start immediately." Per the operating boundary stated at the top of this document, creating and building this repository happens in its own session/workspace, not inside `vcns/security-automation-manager` -- see the kickoff brief, §6.
...
**Resolved, 14 September 2026: Cloudflare Workers, matching `sam-licensing-service`.** Reasoning given: one platform to operate (billing, credentials, observability), and several of that repository's patterns are directly reusable -- the Custom Domain deploy pipeline (`render-wrangler-toml.mjs` + GitHub Actions staging/production), the admin-session auth pattern (HttpOnly/Secure/SameSite=Strict cookie, KV-backed session records, a centrally-audited rejection path), native `[[ratelimits]]` bindings, and its zero-production-dependency discipline. **Explicitly not carried over by default:** `sam-licensing-service`'s Durable-Objects-as-mutation-authority data model. The portal's dashboards/fleet-views/findings-browsing are genuinely relational and query-heavy in a way that repository's mostly write-then-read-by-key model isn't -- whether the portal's core data (tenants, resources, findings, policy versions) should live in D1 instead, with Durable Objects reserved for genuine per-entity strong-consistency needs, is called out as an explicit early decision for the new repository to make, not something this document settles. See the kickoff brief (§6) for the full reasoning as handed to the new session.
...
**Resolved, 14 September 2026: solo, Claude-Code-assisted -- in a new repository and a new chat session, not this one.** Same build model already proven across `security-automation-manager` and `sam-licensing-service`. Per the operating boundary at the top of this document, no portal code is written here; this document (via the kickoff brief, §6) instead prepares what a fresh session in `vcns/sam-portal` needs to start, to be pasted across by hand.

---

# 6. Kickoff Brief for the New Repository

`docs/sam-portal-kickoff-brief.md` (in this repository) is the first message to paste into that fresh session, immediately after pasting the full content of `docs/sam-portal-requirements-spec.md`. It distills this document's build order (§3-§5) plus concrete, named patterns worth reading directly from `sam-licensing-service` (its deploy pipeline docs, its admin-session auth module, its `wrangler.toml`) into a self-contained brief -- written 14 September 2026, so it reflects every decision in §5 as resolved. It is not a substitute for this document, which remains the living tracker on the `security-automation-manager` side; the brief is a one-time export for a session that otherwise starts from nothing.
