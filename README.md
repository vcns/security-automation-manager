...
- Information Masking: per-surface on/off, removing the `X-Powered-By`, `Server`, and `X-Pingback` headers
- Cache-Control: per-surface preset (`no-store` / `private-no-cache` / `public-short` / `public-long`). Defers automatically if another caching mechanism (a caching plugin or CDN) is already active, rather than sending a competing value
