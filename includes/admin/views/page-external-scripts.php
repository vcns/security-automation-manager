<?php
/**
 * Admin view: External Scripts -- third-party <script>/<link rel=stylesheet>
 * origin inventory, classification, and Subresource Integrity.
 * Rendered by Admin_UI::render_external_scripts().
 */
...
$base_url = admin_url( 'admin.php?page=security-automation-manager-external-scripts' );
...
<div class="wrap wp-sam-wrap">
	<h1><?php esc_html_e( 'External Scripts', 'security-automation-manager' ); ?></h1>

	<p>
		<?php esc_html_e( 'Passively inventories every third-party <script> and <link rel="stylesheet"> origin observed on real page loads for an enabled surface -- never a dedicated crawl or extra request. A newly discovered origin starts Unclassified: it is neither trusted nor blocked until you decide. Only the origin (scheme and host) is ever stored, never a path or query string.', 'security-automation-manager' ); ?>
	</p>
	...
	<details class="wp-sam-filter-form">
		...
		<form method="get" action="">
			<input type="hidden" name="page" value="security-automation-manager-external-scripts" />
			<label>
	...
	<table class="widefat fixed striped" style="margin-top:1em">
		<thead>
			<tr>
				...
				<?php echo Table_Query::sort_header( __( 'Origin', 'security-automation-manager' ), 'origin', $ext_sort_whitelist, $ext_sort, $ext_state_args, $base_url, 'ext_paged' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				...
				<?php echo Table_Query::sort_header( __( 'Evidence', 'security-automation-manager' ), 'evidence', $ext_sort_whitelist, $ext_sort, $ext_state_args, $base_url, 'ext_paged' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				<?php echo Table_Query::sort_header( __( 'Last Seen', 'security-automation-manager' ), 'last_seen', $ext_sort_whitelist, $ext_sort, $ext_state_args, $base_url, 'ext_paged' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			</tr>
		</thead>
		<tbody>
	...
	<?php echo Table_Query::pagination( $ext_page_num, $ext_pages, $ext_state_args, $base_url, 'ext_paged' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
</div>
