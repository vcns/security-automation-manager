...
$tab_help = array(
	'start-here' => array(
		...
		'description' => __( 'What this page does for scripts and stylesheets you control, and for the ones you don\'t.', 'security-automation-manager' ),
	),
	'external'   => array(
		'label'       => __( 'External', 'security-automation-manager' ),
		'description' => __( 'Third-party script/stylesheet origins: inventory, classification, and admin-supplied Subresource Integrity.', 'security-automation-manager' ),
	),
	'internal'   => array(
		'label'       => __( 'Internal', 'security-automation-manager' ),
		'description' => __( 'Automatic Subresource Integrity for your own theme, plugin, and core scripts/stylesheets.', 'security-automation-manager' ),
	),
);
...
<div class="wrap wp-sam-wrap">
	<h1><?php esc_html_e( 'Scripts', 'security-automation-manager' ); ?></h1>

	<!-- ── Tabs ──────────────────────────────────────────────────────────── -->
	<nav class="nav-tab-wrapper wp-sam-tab-wrapper" role="tablist" aria-label="<?php esc_attr_e( 'Scripts sections', 'security-automation-manager' ); ?>">
	...
	<h2><?php esc_html_e( 'External scripts and stylesheets', 'security-automation-manager' ); ?></h2>
	<p>
		<?php esc_html_e( 'Every third-party <script> and <link rel="stylesheet"> origin this site loads -- analytics, marketing pixels, embedded widgets, CDN-hosted libraries -- is passively inventoried the moment a real visitor\'s page load includes it. Only the origin (scheme and host) is ever recorded, never a path or query string, and never from a dedicated crawl.', 'security-automation-manager' ); ?>
	</p>
	<p>
		<?php esc_html_e( 'A freshly discovered origin is always Unclassified: neither trusted nor blocked until an administrator decides. You can pin a Subresource Integrity hash for a version-locked script you\'ve reviewed ("Approved -- immutable"), mark a script whose content legitimately changes over time as trusted without SRI ("Approved -- mutable provider"), grant a one-off exception, or block it outright. Report mode (the default) never removes anything; enforce mode removes only what you\'ve explicitly blocked, or an "immutable" origin whose SRI hash no longer matches.', 'security-automation-manager' ); ?>
	</p>
	<p>
		<?php esc_html_e( 'This plugin never fabricates a hash for a third-party script. A pinned SRI hash only ever comes from what you paste in yourself -- a local copy of the file you already trust, the vendor\'s own published hash, or the "Suggest" helper, which fetches a URL you supply and computes its hash for you to review. It is never computed by silently fetching the remote script and trusting whatever comes back, which would defeat the entire point of SRI if that remote origin were ever compromised.', 'security-automation-manager' ); ?>
	</p>

	<h2><?php esc_html_e( 'Internal scripts and stylesheets', 'security-automation-manager' ); ?></h2>
	<p>
		<?php esc_html_e( 'Your own theme, plugin, and WordPress core scripts and stylesheets are a fundamentally different case: this server already has the exact file it\'s about to send. When enabled for a surface, this plugin reads that file directly, computes its SHA-384 hash, and adds it as the script or stylesheet tag\'s integrity attribute automatically -- no manual review, no pasted hash, no remote fetch of any kind. The hash is recalculated the moment a file\'s size or modified time changes, so a plugin/theme update is reflected on the very next page load that serves it -- nothing to remember to update by hand.', 'security-automation-manager' ); ?>
	</p>
	<p>
		<?php esc_html_e( 'This is safe in a way a third-party hash never can be: it never trusts a remote party to compute anything, so there is no compromised-CDN scenario to worry about. What it does protect against is the file being altered after this server serves it -- a tampered cache, a compromised CDN sitting in front of this site, or similar -- the same class of threat SRI addresses for third-party assets, just without ever needing a third party in the loop.', 'security-automation-manager' ); ?>
	</p>
	<p class="description">
		<?php esc_html_e( 'Off by default per surface, like every other pillar in this plugin -- nothing changes until you turn it on for a surface on the Internal tab.', 'security-automation-manager' ); ?>
	</p>
